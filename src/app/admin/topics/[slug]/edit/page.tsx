...
import { updateTopic } from "@/lib/admin-actions";
...
export default function EditTopicPage() {
  ...
  const [form, setForm] = useState({ title: "", slug: "", description: "", status: "draft", order: "0" });

  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/admin/topics/${slug}`);
        if (!res.ok) throw new Error("موضوع یافت نشد");
        ...
        setError(e instanceof Error ? e.message : "خطا در بارگذاری موضوع");
      } finally {
        setFetching(false);
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await updateTopic(slug, sha, formData);
      router.push("/admin/topics");
      router.refresh();
    } catch (err) {
      alert(err instanceof Error ? err.message : "خطا در ویرایش موضوع");
    } finally {
      setLoading(false);
    }
  }
