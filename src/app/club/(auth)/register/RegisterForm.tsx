...
import { ArrowRight, Mail, Users, Lock } from "lucide-react";

const initialRegister = { error: "" };
const initialWaitlist = { success: false };

const inputClass =
  "w-full px-3.5 py-2.5 border border-border bg-background text-foreground text-sm rounded-xl focus:outline-hidden focus:border-accent transition-colors font-mono";
        ...
        <div className="border border-border bg-surface p-6 sm:p-8 rounded-2xl shadow-xs">
          ...
          <form action={regAction} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <input name="username" placeholder="مثال: socrates" required className={inputClass} />
            </div>
            ...
            {regState?.error && <p className="text-xs text-red-600 dark:text-red-400 font-medium">{regState.error}</p>}
        ...
        <div className="border border-border bg-surface p-6 sm:p-8 rounded-2xl shadow-xs">
          ...
          <form action={regAction} className="flex flex-col gap-4">
            ...
            <button
              ...
              className="w-full py-2.5 text-sm rounded-xl bg-accent text-accent-fg font-semibold hover:opacity-90 disabled:opacity-50 transition-opacity mt-2 shadow-xs cursor-pointer"
            >
              {regPending ? "در حال ایجاد حساب..." : "ایجاد حساب"}
            </button>
          </form>
      ...
      <div className="border border-border bg-surface p-6 sm:p-8 rounded-2xl shadow-xs">
        ...
        {wlState.success ? (
          ...
          <form action={wlAction} className="flex flex-col gap-3">
            ...
            {wlState?.error && <p className="text-xs text-red-600 dark:text-red-400 font-medium">{wlState.error}</p>}
            <button
              type="submit"
              disabled={wlPending}
